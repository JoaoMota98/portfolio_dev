...
async function getRepos(){ 
    ...
    const htmlProjetos = meusFavoritos.map(projeto => {
        ...
        const nomeFormatado = projeto.name.replace(/[_-]/g, ' ').replace(/\b\w/g, l => l.toUpperCase());

        const tags = projeto.topics
        .filter(topic => topic !== 'portfolio') // Remove o 'portfolio' da exibição
        .map(topic => `
            <span class="bg-white/5 text-gray-400 text-[10px] font-medium px-2 py-1 rounded border border-white/10 uppercase tracking-wider">
                ${topic}
            </span>
        `).join('');
        ...
        <div class="flex-none w-full sm:w-[350px] backdrop-blur-md p-6 rounded-2xl border border-white/10 hover:border-[#7988f9] hover:border-2 transition-all flex flex-col items-center hover:scale-105 hover:shadow-[0_0_20px_#7988f9] gap-3">
            ...
            <h3 class="w-full sm:w-[350px] text-[22px] font-bold text-white mb-2 text-center">${nomeFormatado}</h3>
...
menuBtn.addEventListener('click', () => {
    // Alterna o menu entre escondido e visível (modo coluna no mobile)
    menuLinks.classList.toggle('hidden');
    menuLinks.classList.toggle('flex');
});

// Fecha o menu automaticamente quando clicar em um link (melhor UX)
